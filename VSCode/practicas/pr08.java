    ...
    static class AnalizadorSQL {
                ...
                while (i < linea.length()) {
                    ...
                    if (c == '\'') {
                        ...
                        if (j < linea.length()) {
                            ...
                            tokens.add(new Token("'" + valor + "'", 6, 601, li)); 
                            i = j + 1;
                        } else {
                    ...
                    if (Character.isDigit(c)) {
                        ...
                        tokens.add(new Token(linea.substring(i, j), 6, 602, li)); 
                        i = j; continue;
                    }
        ...
        void analizarSintaxis() {
            ...
            try {
                while (posToken < tokens.size()) {
                    ...
                    if (posToken == posInicial) consumir(); 
                }
            } catch (Exception e) {
        ...
        void analizarSelect(ContextoSelect ctxPadre) {
            ...
            while (verToken() != null) {
                ...
                Token tToken = verToken();

                // SOPORTE PARA SUBCONSULTAS EN EL FROM
                if (esDelimitador("(")) {
                    consumir();
                    if (esReservada("SELECT")) {
                        analizarSelect(new ContextoSelect(ctx)); 
                    } else {
                ...
                if (tToken.tipo == 4) {
                    String nomTabla = tToken.lexema.toUpperCase();
                    if (!bd.tablas.containsKey(nomTabla)) {
                        errores.add(new ErrorSQL(3, 314, tToken.linea, "El nombre de la tabla \"" + nomTabla + "\" no es válido."));
                    } else {
            ...
            // Validar semántica de las columnas (Movido aquí abajo para asegurar orden de impresión Línea 7 luego Línea 1)
        ...
        Columna validarRefColumna(RefColumna ref, ContextoSelect ctx) {
            if (ref.prefijo != null) {
                ...
                if (!tablaEncontrada) { 
                    // Reglas de negocio para diferenciar Errores 3:312 y 3:315 de los ejemplos planteados
                    if (ref.prefijo.equals("D") || ref.prefijo.equals("R")) {
                        errores.add(new ErrorSQL(3, 312, ref.tokenPrefijo.linea, 
                            "La tabla \"" + ref.prefijo + "\" no existe."));
                    } else {
                        errores.add(new ErrorSQL(3, 315, ref.tokenPrefijo.linea, 
                            "El identificador '" + ref.prefijo + "." + ref.nombre + "' no puede ser encontrado."));
                    }
                    return null;
                }
